...
class XBEDaemon(Daemon):
    ...
    def configure(self):
        ...
        if self.opts.price is not None:
            self.price = float(self.opts.price)
        elif cp.has_option("broker", "price"):
            self.price = float(cp.get("broker", "price"))
        else:
            self.price = 0.0

	if os.environ.get("XBED_DEBUG", False):
	        print >>sys.stderr, "Configuration done."
	...
	if os.environ.get("XBED_DEBUG", False):
        	print >>sys.stderr, "Logging configured."
        global log
