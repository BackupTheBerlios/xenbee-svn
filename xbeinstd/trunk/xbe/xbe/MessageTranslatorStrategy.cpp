...
TaskData
MessageTranslatorStrategy::parseTaskDescription(xbemsg::execute_t::task_type &task) {
...
void MessageTranslatorStrategy::perform(const seda::IEvent::Ptr& e) {
    if (xbe::event::ObjectEvent<xbemsg::message_t> *oe = (dynamic_cast<xbe::event::ObjectEvent<xbemsg::message_t>*>(e.get()))) {
        ...
        seda::IEvent::Ptr evtPtr((seda::IEvent*)0);

        if (msg->body().execute()) {
            XBE_LOG_INFO("translating execute");
            evtPtr.reset(new xbe::event::ExecuteEvent(to, from, convID));
            xbe::event::ExecuteEvent *evt(static_cast<xbe::event::ExecuteEvent*>(evtPtr.get()));

            // parse the jsdl description
            if (msg->body().execute()->task()) {
                evt->taskData(parseTaskDescription(*msg->body().execute()->task())); 
            }
            if (msg->body().execute()->status()) {
                evt->statusTaskData(parseTaskDescription(*msg->body().execute()->status())); 
            }

            seda::StrategyDecorator::perform(evtPtr);
            ...
            xbe::event::LifeSignEvent *evt(new xbe::event::LifeSignEvent(to, from, convID, msg->body().life_sign()->timestamp()));
            ...
            xbe::event::StatusEvent *evt(new xbe::event::StatusEvent(to, from, convID,
                                                                     msg->body().status()->timestamp()));
            if (msg->body().status()->stdout()) {
                std::string stdout(msg->body().status()->stdout()->data(), msg->body().status()->stdout()->size());
                evt->taskStatusStdOut(stdout);
            }
            if (msg->body().status()->stderr()) {
                std::string stderr(msg->body().status()->stderr()->data(), msg->body().status()->stderr()->size());
                evt->taskStatusStdErr(stderr);
            }
            seda::StrategyDecorator::perform(seda::IEvent::Ptr(evt));
        } else if (msg->body().finished()) {
            XBE_LOG_INFO("translating finished");
            xbe::event::FinishedEvent *evt(new xbe::event::FinishedEvent(to, from, convID, msg->body().finished()->exitcode()));
        ...
        if (xbe::event::ExecuteEvent *evt = dynamic_cast<xbe::event::ExecuteEvent*>(xbeEvent)) {
            throw xbe::TranslationError("API error - client is not supposed to translate ExecuteEvents to xml: " + evt->str());
        } else if (xbe::event::TerminateEvent *evt = dynamic_cast<xbe::event::TerminateEvent*>(xbeEvent)) {
            throw xbe::TranslationError("API error - client is not supposed to translate TerminateEvents to xml: " + evt->str());
            ...
            xbemsg::life_sign_t life_sign(evt->timestamp());
            ...
            xbemsg::status_t status(evt->timestamp(), evt->taskStatusCode());
            status.stdout(xbemsg::status_t::stdout_type(evt->taskStatusStdOut().c_str(), evt->taskStatusStdOut().size()));
            status.stderr(xbemsg::status_t::stderr_type(evt->taskStatusStdErr().c_str(), evt->taskStatusStdErr().size()));
            body.status(status);

            std::auto_ptr<xbemsg::message_t> msg(new xbemsg::message_t(header, body));
            seda::StrategyDecorator::perform(seda::IEvent::Ptr(new xbe::event::ObjectEvent<xbemsg::message_t>(to, from, msg)));
        } else if (xbe::event::FinishedEvent *evt = dynamic_cast<xbe::event::FinishedEvent*>(xbeEvent)) {
            xbemsg::finished_t finished(evt->exitcode());
